...
import { fromStroops } from "@/lib/stellar/phpx";
...
export default function CoopPage() {
  ...
  useEffect(() => {
    ...
    // (the PHPx issuer). It can observe, but never holds custody of, earnings.
    fetchRides(STELLAR.phpxIssuer)
      .then(setRides)
    ...
    <>
      ...
      <main className="flex flex-1 flex-col gap-4 px-5 py-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Stat label="Fares settled" value={php(stats.settled)} brand />
          <Stat label="Auto-refunded" value={php(stats.refunded)} />
            ...
            <div key={r.id} className="card">
              ...
              <div className="mt-2 flex items-center justify-between text-sm">
                <span className="text-slate-500">
                  Locked {php(Number(fromStroops(r.maxFare)))}
                </span>
                <span className="font-bold text-brand-dark">
                  {r.status === "Completed"
                    ? `Paid ${php(Number(fromStroops(r.actualFare)))}`
                    : "—"}
                </span>
              </div>
            </div>
